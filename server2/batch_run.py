...
try:
    ...
    from eval_core import summarize_binary_results
...
def parse_record(obj: dict) -> Optional[dict]:
    ...
    passthrough = {}
    for key in ["dataset", "predicted_attack", "sample_id"]:
        if key in obj:
            passthrough[key] = obj.get(key)
...
def _detail_row_passthrough(row) -> Dict:
    passthrough = {}
    for key in ["dataset", "predicted_attack", "sample_id"]:
        if key in row:
            passthrough[key] = row.get(key)
    return passthrough
...
def write_df_to_jsonl(df: pd.DataFrame, out_path: str):
    ...
    with open(out_path, "w", encoding="utf-8-sig") as f:
        for _, r in df.iterrows():
            ...
            obj.update(_detail_row_passthrough(r))
...
def compute_metrics(df: pd.DataFrame) -> dict:
    ...
    if {"dataset", "predicted_attack"}.issubset(df.columns):
        out["binary_summary"] = summarize_binary_results(df.to_dict("records"))
...
def run_infer_on_dir(
    ...
    for i, filename in enumerate(tqdm(jsonl_files, desc=f"处理文件夹 {os.path.basename(input_dir)}"), 1):
        ...
        for _, r in src_df.iterrows():
            ...
            detail_row.update(_detail_row_passthrough(r))
